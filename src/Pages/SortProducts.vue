<template>
  <section class="relative">
    ...
    >
      <ProductInfo
        v-for="pro in sortedProducts.slice(0, 5)"
        :key="pro"
        :productDetails="pro"
      />
    </div>
...
import { useProductInSort } from "../store/productsort";

export default {
  name: "SortProducts",
  components: { ProductsGallery },
  setup() {
    const { products, sortedProducts, retrieveProducts } = useProductInSort();
    onMounted(async () => await retrieveProducts());
